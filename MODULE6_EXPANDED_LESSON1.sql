-- MODULE 6 COMPLETE - ALL 3 LESSONS EXPANDED
...
<p>Distribute budgets across funnel stages based on business maturity and goals. Calculate required budgets by working backwards from business goals, considering customer lifetime value and market competition. Strategic allocation and disciplined scaling maximize return on advertising investment.</p>
    ...
    (new_lesson_id, 'What is the 70-20-10 budget allocation rule?', '["70% proven campaigns, 20% promising, 10% experimental", "70% awareness, 20% consideration, 10% conversion", "70% testing, 20% scaling, 10% maintenance", "70% Facebook, 20% Instagram, 10% other"]'::jsonb, 0, '70-20-10 allocates most budget to proven winners while maintaining testing for new opportunities'),
    (new_lesson_id, 'How many conversions should testing budgets generate?', '["50-100 conversions for statistical significance", "10 conversions minimum", "5 conversions is enough", "1,000 conversions required"]'::jsonb, 0, '50-100 conversions provides reliable data for test conclusions'),
    (new_lesson_id, 'What is the recommended budget scaling pace?', '["20-30% increases every 3-4 days", "Double budget immediately", "10% daily increases", "50% weekly increases"]'::jsonb, 0, 'Gradual 20-30% increases maintain optimization stability when scaling'),
    (new_lesson_id, 'When should you pause budget scaling?', '["When cost per result increases more than 30%", "Never pause scaling", "After any cost increase", "When budget doubles"]'::jsonb, 0, 'Cost increases over 30% indicate problems requiring investigation before further scaling'),
    (new_lesson_id, 'How should budget allocation evolve with business maturity?', '["New businesses need more testing, mature businesses focus on proven winners", "Always use same allocation", "Mature businesses test more", "New businesses only run proven campaigns"]'::jsonb, 0, 'Allocation should shift from exploration (new businesses) to exploitation (mature businesses)');
    
    RAISE NOTICE 'Module 6 Lesson 1 created';
