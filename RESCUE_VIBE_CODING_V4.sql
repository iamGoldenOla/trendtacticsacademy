-- ============================================================================
-- RESCUE VIBE CODING V4 (Deduplication & Final Fix)
-- ============================================================================
-- 1. DELETES duplicate lessons first.
--    (The error occurred because multiple "UI / Frontend AI" lessons existed, 
--    so the UPDATE tried to set ALL of them to Ordering=1, causing a crash.)
-- 2. Resets ordering of survivors to 5000+.
-- 3. Updates the single survivor with correct Module + Video.
-- ============================================================================
...
DECLARE
    ...
    -- List of titles to deduplicate
    titles text[] := ARRAY[
        'Intent Is the New Programming%', 
        'What Is Vibe Coding%', 
        'From Writing Code%',
        'How AI Thinks%',
        'Context, Memory%',
...
BEGIN
    ...
    -- ========================================================================
    -- STEP 1: DEDUPLICATE (The Fix for "Duplicate Key")
    -- ========================================================================
    RAISE NOTICE 'Step 1: Removing Duplicates...';
    
    FOREACH t IN ARRAY titles
    LOOP
        -- Delete all matches except the one with the most recent created_at
        DELETE FROM lessons 
        WHERE id IN (
            SELECT id FROM (
                SELECT id, ROW_NUMBER() OVER(PARTITION BY title ORDER BY created_at DESC) as rnum
                FROM lessons 
                WHERE title ILIKE t
            ) sub
            WHERE sub.rnum > 1
    ...
    -- ========================================================================
    -- STEP 3: RESET ORDERING (Clear the Slots)
    -- ========================================================================
    RAISE NOTICE 'Step 3: Moving existing lessons out of the way...';
    
    -- Move everyone in these modules to 5000+
    UPDATE lessons 
    SET ordering = ordering + 5000 
    WHERE module_id IN (m1_id, m2_id, m3_id, m4_id, m5_id, m6_id, m7_id, m8_id);

    -- ========================================================================
    -- STEP 4: SLOT IN & UPDATE VIDEOS
    -- ========================================================================
    RAISE NOTICE 'Step 4: Slotting lessons...';
