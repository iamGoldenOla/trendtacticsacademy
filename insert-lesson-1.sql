...
-- First, get the module ID for "Understanding Vibe Coding"
DO $$
DECLARE
    v_module_id UUID;
    v_lesson_json JSONB;
BEGIN
    ...
    FROM modules
    WHERE course_id = 'e9e58e4a-2f3b-4c9a-8a3d-1e5f6a7b8c9d'
    AND title = 'Understanding Vibe Coding';

    -- Prepare the lesson JSON
    v_lesson_json := '{
    ...
    ON CONFLICT (module_id, lesson_number)
    DO UPDATE SET
        lesson_json = EXCLUDED.lesson_json,
        updated_at = NOW();

    RAISE NOTICE 'Lesson 1 inserted successfully!';
END $$;
