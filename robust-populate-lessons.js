...
async function populateLessons() {
  try {
    ...
    // Insert all lessons one by one to get better error reporting
    let successCount = 0;
    for (let i = 0; i < lessons.length; i++) {
      const lesson = lessons[i];
      console.log(`   Inserting lesson ${i+1}/${lessons.length}: "${lesson.title}"`);
      ...
      if (error) {
        console.log(`❌ Error inserting lesson "${lesson.title}":`, error.message);
    ...
    console.log(`\n📊 Results: Successfully inserted ${successCount} out of ${lessons.length} lessons`);
    ...
    for (const module of modules) {
      ...
      if (countError) {
        ...
        totalVerifiedLessons += count;
      }
    }

    console.log(`\n📊 Total verified lessons: ${totalVerifiedLessons}`);

    if (successCount === lessons.length && totalVerifiedLessons === lessons.length) {
      console.log('\n🎉 All lessons successfully populated!');
      console.log('The course should now appear on your website.');
      console.log('\n🔄 Please refresh your website to see the changes.');
    } else {
      console.log('\n⚠️  There were issues with lesson insertion.');
