-- Supabase Schema for Vibe Coding Course
...
CREATE INDEX idx_lessons_lesson_number ON lessons(module_id, lesson_number);
CREATE INDEX idx_student_progress_student_id ON student_progress(student_id);
CREATE INDEX idx_student_progress_lesson_id ON student_progress(lesson_id);

-- Insert the Vibe Coding course
INSERT INTO courses (title, description) 
VALUES ('Vibe Coding', 'A beginner-friendly approach to creating digital products by focusing on ideas, intention, creativity, and guidance with AI assistance');
...
BEGIN
    ...
    (course_id_var, 'Understanding Vibe Coding', 1),
    (course_id_var, 'Thinking Before Coding', 2),
    (course_id_var, 'Working with AI as a Creative Partner', 3),
    (course_id_var, 'Vibe Coding in Real Life', 4),
    (course_id_var, 'Exploring and Building with Confidence', 5);
END $$;

-- Insert the generated lessons
DO $$
DECLARE
    course_id_var UUID;
    module_id_var UUID;
    lesson_number_var INTEGER := 1;
BEGIN
    SELECT id INTO course_id_var FROM courses WHERE title = 'Vibe Coding' LIMIT 1;
    
    -- Insert Understanding Vibe Coding module lessons
    ...
    -- Insert Thinking Before Coding module lessons
