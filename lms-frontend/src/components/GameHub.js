...
const GameHub = ({ courseId, courseTitle, gameActivities, onGameComplete, onClose }) => {
    ...
    const getCompletionBadge = (activity) => {
        if (!activity.completed)
            return null;
        const score = activity.bestScore || 0;
        if (score >= 90)
            return { emoji: '🏆', text: 'Gold', color: 'text-yellow-600 bg-yellow-100' };
        if (score >= 75)
            return { emoji: '🥈', text: 'Silver', color: 'text-gray-600 bg-gray-100' };
        if (score >= 60)
            return { emoji: '🥉', text: 'Bronze', color: 'text-orange-600 bg-orange-100' };
        return { emoji: '✅', text: 'Complete', color: 'text-green-600 bg-green-100' };
    };

    if (selectedActivity) {
        return (<GameActivity activity={selectedActivity} onComplete={handleGameComplete} onClose={() => setSelectedActivity(null)}/>);
    }
      ...
      <div className="bg-white rounded-xl max-w-6xl w-full mx-4 shadow-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            return (<div key={activity.id} className={`border-2 rounded-xl p-6 transition-all duration-200 cursor-pointer ${activity.unlocked
                    ? 'border-gray-200 hover:border-brand-cyan hover:shadow-lg'
                    : 'border-gray-100 bg-gray-50 cursor-not-allowed opacity-60'}`} onClick={() => handleActivitySelect(activity)}>
                  {/* Activity Header */}
                  <div className="flex items-start justify-between mb-4">
                    <div className="text-3xl">{getTypeIcon(activity.type)}</div>
                    <div className="flex flex-col items-end space-y-2">
                      {!activity.unlocked && (<span className="text-xs text-gray-500 bg-gray-200 px-2 py-1 rounded-full">
                          🔒 Locked
                        </span>)}
                      {completionBadge && (<span className={`text-xs px-2 py-1 rounded-full border ${completionBadge.color}`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {activity.timeLimit && (<span className="text-xs text-gray-500">
                        ⏱️ {Math.floor(activity.timeLimit / 60)}m
                      </span>)}
                  </div>
                  
                  {/* Best Score */}
                  {activity.completed && activity.bestScore && (<div className="mt-3 pt-3 border-t border-gray-200">
                  ...
                  {/* Play Button */}
                  {activity.unlocked && (<button className="w-full mt-4 bg-brand-cyan text-white py-2 px-4 rounded-lg font-medium hover:bg-brand-navy transition-colors">
          ...
          <div className="mt-8 p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl border border-purple-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className={`text-center p-3 rounded-lg ${userStats.gamesCompleted >= 1 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
                <div className="text-2xl mb-1">🎯</div>
                <div className="text-xs font-medium">First Game</div>
              </div>
              <div className={`text-center p-3 rounded-lg ${userStats.totalPoints >= 100 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-400'}`}>
                <div className="text-2xl mb-1">💯</div>
                <div className="text-xs font-medium">100 Points</div>
              </div>
              <div className={`text-center p-3 rounded-lg ${userStats.gamesCompleted >= 5 ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-400'}`}>
                <div className="text-2xl mb-1">🔥</div>
                <div className="text-xs font-medium">Game Master</div>
              </div>
              <div className={`text-center p-3 rounded-lg ${userStats.averageScore >= 90 ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-400'}`}>
                <div className="text-2xl mb-1">⭐</div>
