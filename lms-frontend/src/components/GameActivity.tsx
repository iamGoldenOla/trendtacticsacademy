...
import type { GameActivity, GameQuestion } from '../services/types';
...
const GameActivity: React.FC<GameActivityProps> = ({ activity, onComplete, onClose }) => {
  ...
  const [memoryCards, setMemoryCards] = useState<{ id: string; content: string; flipped: boolean; matched: boolean }[]>([]);
  ...
  const handleMemoryCardClick = (cardId: string) => {
    ...
    const newMemoryCards = memoryCards.map(card => 
    ...
    if (newFlippedCards.length === 2) {
      setTimeout(() => {
        ...
        const isMatch = firstCard && secondCard && (
          (first.startsWith('term-') && second.startsWith('def-') && first.split('-')[1] === second.split('-')[1]) ||
        ...
        <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4 shadow-2xl">
          <div className="text-center">
            ...
            <div className="flex space-x-3">
              <button
                onClick={startGame}
                className="flex-1 bg-brand-cyan text-white py-3 px-6 rounded-lg font-semibold hover:bg-brand-navy transition-colors"
              >
                Start Game
              </button>
              <button
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4 shadow-2xl">
          <div className="text-center">
            ...
            <button
              onClick={onClose}
              className="w-full bg-brand-cyan text-white py-3 px-6 rounded-lg font-semibold hover:bg-brand-navy transition-colors"
            >
              Continue Learning
            </button>
          </div>
          ...
          {!showExplanation ? (
            <>
              ...
              <button
                ...
                className="w-full bg-brand-cyan text-white py-3 px-6 rounded-lg font-semibold hover:bg-brand-navy transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
              >
            ...
            <>
              ...
              <div className={`p-4 rounded-lg mb-6 ${
                ...
                <div className="flex items-center mb-2">
                  {selectedAnswers[currentQuestionIndex] === currentQuestion.correctAnswer ? (
                    <span className="text-green-600 font-semibold">✅ Correct! (+{currentQuestion.points} points)</span>
                  ) : (
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            {memoryCards.map((card) => (
              <button
                ...
                className={`aspect-square p-4 rounded-lg border-2 transition-all duration-300 ${
                  card.matched
          ...
          <div className="grid grid-cols-2 gap-8 mb-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                {['Paid Advertising', 'Organic Marketing', 'Direct Marketing'].map((zone) => (
                  <div
                    key={zone}
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={(e) => handleDrop(e, zone)}
                    className="p-4 border-2 border-dashed border-gray-300 rounded-lg min-h-[60px] hover:border-gray-400 transition-colors"
                  >
