...
const LessonViewer: React.FC<LessonViewerProps> = ({ course, lessonId, onLessonComplete }) => {
  ...
  const [quizInterval, setQuizInterval] = useState<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    if (!currentLesson || !currentLesson.quiz || quizCompleted) return;
  ...
  const quizScore = currentLesson && currentLesson.quiz ? quizAnswers.filter((a, i) => a === currentLesson.quiz?.[i].correctAnswer).length : 0;
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
              ...
              {currentLesson && currentLesson.quiz && (
                <div className="bg-white rounded-lg shadow-md p-6 mb-6 mt-4 border border-blue-100">
                  ...
                  {!quizCompleted ? (
                    <>
                      ...
                      <div className="space-y-2">
                        {currentLesson.quiz[quizStep].options.map((option: string, idx: number) => (
                          <button
                            key={idx}
                            onClick={() => handleQuizAnswer(idx)}
                            className={`block w-full text-left px-4 py-2 rounded-lg border transition-colors
                              ${quizAnswers[quizStep] === idx ? (idx === currentLesson.quiz?.[quizStep].correctAnswer ? 'bg-green-100 border-green-400 text-green-800' : 'bg-red-100 border-red-400 text-red-800') : 'bg-gray-50 border-gray-200 hover:bg-blue-50'}`}
                      ...
                      {quizAnswers[quizStep] !== null && (
                        <div className={`mt-4 font-semibold ${quizAnswers[quizStep] === currentLesson.quiz[quizStep].correctAnswer ? 'text-green-700' : 'text-red-700'}`}>
                          {quizAnswers[quizStep] === currentLesson.quiz[quizStep].correctAnswer ? 'Correct! 🎉' : 'Incorrect.'}
                          {currentLesson.quiz[quizStep].explanation && (
                            <div className="text-sm mt-1 text-gray-600">{currentLesson.quiz[quizStep].explanation}</div>
                          )}
                        </div>
                      )}
                      <div className="flex justify-between mt-6">
                        <button onClick={handleQuizPrev} disabled={quizStep === 0} className="btn-secondary disabled:opacity-50">Previous</button>
                        {quizStep < currentLesson.quiz.length - 1 ? (
                          <button onClick={handleQuizNext} disabled={quizAnswers[quizStep] === null} className="btn-primary disabled:opacity-50">Next</button>
                        ) : (
                          <button onClick={handleQuizNext} disabled={quizAnswers[quizStep] === null} className="btn-primary disabled:opacity-50">Finish</button>
                        )}
                      </div>
                    </>
                  ) : (
            ...
            {showNotes && (
              <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                ...
                <textarea
                  ...
                  placeholder="Take notes during this lesson..."
                  className="w-full h-32 p-3 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-brand-cyan focus:border-transparent"
                />
