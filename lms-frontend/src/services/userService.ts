...
export const getUserProfile = async (): Promise<User | null> => {
  try {
    const response = await get<ApiResponse<User>>(USER_ENDPOINTS.GET_PROFILE);
...
export const updateUserProfile = async (userData: Partial<User>): Promise<User | null> => {
  try {
    const response = await put<ApiResponse<User>>(USER_ENDPOINTS.UPDATE_PROFILE, userData);
    if (response.success && response.data) {
...
export const updatePassword = async (currentPassword: string, newPassword: string): Promise<boolean> => {
  try {
    const response = await put<ApiResponse<any>>(USER_ENDPOINTS.UPDATE_PASSWORD, {
      currentPassword,
      newPassword,
    });
