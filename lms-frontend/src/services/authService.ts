...
import { ApiResponse, AuthResponse, LoginCredentials, SignupData, User } from '../types/api';
...
export const login = async (credentials: LoginCredentials): Promise<AuthResponse> => {
  try {
    ...
    const response = await post<User & { token: string }>(AUTH_ENDPOINTS.LOGIN, credentials);
...
export const signup = async (userData: SignupData): Promise<AuthResponse> => {
  try {
    ...
    const response = await post<User & { token: string }>(AUTH_ENDPOINTS.SIGNUP, userData);
...
export const updateProfile = async (profileData: Partial<User>): Promise<User> => {
  try {
    const updatedUser = await post<User>(AUTH_ENDPOINTS.CURRENT_USER, profileData);
