...
const CourseDetail = ({ user, onLogin, onSignup }) => {
    ...
    return (<div className="min-h-screen bg-gray-50">
      {/* Success Message */}
      {showSuccessMessage && (<div className="fixed top-4 right-4 z-50 bg-green-500 text-white px-6 py-4 rounded-lg shadow-lg">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6 sticky top-4">
                ...
                {isEnrolled ? (<div className="flex flex-col gap-3">
                    <Link to={`/course/${course.id}/lesson/${course.lessons[0].id}`} className="w-full btn-primary text-center" state={{ fromDashboard: false }}>
                      Continue Learning
                    </Link>
                    <Link to="/dashboard" className="w-full btn-secondary text-center">
                      Go to Dashboard
                    </Link>
                  </div>) : (<button onClick={handleEnroll} className="w-full btn-primary">
                    Enroll Now
                  </button>)}
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {activeTab === 'curriculum' && (<div>
              <h2 className="text-2xl font-heading font-bold text-gray-900 mb-6">
                Course Curriculum
              </h2>
              <div className="space-y-4">
                {course.lessons.map((lesson, index) => (<div key={lesson.id} className="border border-gray-200 rounded-lg p-4 hover:border-gray-300 transition-colors">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        {isEnrolled ? (<Link to={`/course/${course.id}/lesson/${lesson.id}`} className="text-brand-cyan hover:text-brand-navy font-medium text-sm" state={{ fromDashboard: false }}>
                            {lesson.isCompleted ? 'Review' : 'Start'}
                          </Link>) : (<span className="text-gray-400 text-sm">Preview</span>)}
                      </div>
                    </div>
                  </div>))}
              </div>
              {course.moduleQuiz && course.moduleQuiz.length > 0 && (<div className="mt-8 text-center">
          ...
          {activeTab === 'instructor' && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2">
                <h2 className="text-2xl font-heading font-bold text-gray-900 mb-6">
          ...
          {activeTab === 'reviews' && (<div>
            ...
            ].map((review, index) => (<div key={index} className="border border-gray-200 rounded-lg p-6">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-1">
                            {[...Array(5)].map((_, i) => (<svg key={i} className={`w-4 h-4 ${i < review.rating ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                              </svg>))}
                          </div>
                        </div>
          ...
          {activeTab === 'games' && (<div>
               ...
               {course.gameActivities && course.gameActivities.length > 0 ? (<div className="space-y-6">
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {course.gameActivities.slice(0, 3).map((activity) => (<div key={activity.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                         <div className="flex items-center justify-between mb-3">
                           <span className="text-2xl">
                             {activity.type === 'quiz' ? '🧠' : activity.type === 'memory' ? '🃏' : '🎯'}
                           </span>
                           <span className={`text-xs px-2 py-1 rounded-full ${activity.difficulty === 'easy' ? 'bg-green-100 text-green-700' :
                        activity.difficulty === 'medium' ? 'bg-yellow-100 text-yellow-700' :
                         ...
                         <div className="flex items-center justify-between text-xs text-gray-500">
                           ...
                           {activity.timeLimit && (<span>⏱️ {Math.floor(activity.timeLimit / 60)}m</span>)}
                         </div>
                       </div>))}
                   </div>
                   
                   <div className="text-center">
                     <button onClick={() => setShowGameHub(true)} className="bg-gradient-to-r from-brand-cyan to-brand-navy text-white px-8 py-3 rounded-lg font-semibold hover:shadow-lg transition-all duration-200 transform hover:scale-105">
                       🎮 Launch Game Hub
                     </button>
                   </div>
      ...
      {/* Lesson Routes */}
      <Routes>
        <Route path="lesson/:lessonId" element={<LessonViewer course={course} onLessonComplete={handleLessonComplete}/>}/>
      </Routes>

      {/* Payment Modal */}
      <PaymentModal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} course={course} onPaymentSuccess={handlePaymentSuccess} onPaymentError={handlePaymentError}/>
      
      {/* Game Hub Modal */}
      {showGameHub && course?.gameActivities && (<GameHub courseId={course.id} courseTitle={course.title} gameActivities={course.gameActivities} onGameComplete={handleGameComplete} onClose={() => setShowGameHub(false)}/>)}
    </div>);
};
