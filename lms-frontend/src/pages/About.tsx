...
const press = [
  'https://upload.wikimedia.org/wikipedia/commons/6/6b/Forbes_logo.svg',
  'https://upload.wikimedia.org/wikipedia/commons/0/0e/TechCrunch_logo.svg',
  'https://upload.wikimedia.org/wikipedia/commons/4/4a/Business_Insider_Logo.svg',
  'https://upload.wikimedia.org/wikipedia/commons/2/2e/CNN_logo.svg',
];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="max-w-4xl mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((stat) => (
            <div key={stat.label} className={`rounded-xl p-8 flex flex-col items-center stats-card border-2 shadow-lg transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl ${statColors[stat.label] || 'bg-white border-gray-200'}`}>
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        ...
        <div className="overflow-x-hidden">
          <div
            ref={carouselRef}
            className="flex gap-8 w-max min-w-full items-center stats-carousel overflow-hidden"
            style={{ scrollBehavior: 'smooth' }}
          >
            {[...testimonials, ...testimonials].map((t, idx) => (
              <div
                key={idx}
                className={`min-w-[320px] max-w-xs rounded-2xl shadow-lg p-6 flex flex-col items-center border-2 stats-card hover:shadow-2xl transition-shadow duration-300 ${testimonialCardColors[idx % testimonialCardColors.length]} overflow-hidden`}
              >
      ...
      <section className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {team.map((member, idx) => (
            <div key={member.name} className={`rounded-2xl shadow-lg p-6 flex flex-col items-center border-2 transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl ${teamCardColors[idx % teamCardColors.length]}`}> 
      ...
      <section className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="overflow-x-hidden">
          <div
            className="flex gap-12 w-max min-w-full items-center stats-carousel"
            style={{ animation: 'stats-scroll 30s linear infinite', perspective: '800px' }}
          >
            {[...partners, ...partners].map((partner, idx) => (
              <a key={partner.name + idx} href={partner.url} target="_blank" rel="noopener noreferrer">
                <div
                  className={`h-20 md:h-28 max-w-[180px] flex items-center justify-center rounded-lg p-4 shadow-lg border-2 transition-transform duration-300 hover:scale-110 ${partnerColors[partner.name] || 'bg-white border-gray-200'}`}
                  style={{ background: '', minWidth: '140px' }}
                >
                  <img
                    src={partner.logo}
