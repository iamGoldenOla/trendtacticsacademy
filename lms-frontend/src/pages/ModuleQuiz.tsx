  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {questions.length === 0 ? (
          <div className="text-center">No module quiz available for this course.</div>
        ) : (
            ...
            {!quizCompleted ? (
              <>
                ...
                <div className="space-y-2">
                  {questions[quizStep].options.map((option: string, idx: number) => (
                    <button
                      key={idx}
                      onClick={() => handleQuizAnswer(idx)}
                      className={`block w-full text-left px-4 py-2 rounded-lg border transition-colors
                        ${quizAnswers[quizStep] === idx ? (idx === questions[quizStep].correctAnswer ? 'bg-green-100 border-green-400 text-green-800' : 'bg-red-100 border-red-400 text-red-800') : 'bg-gray-50 border-gray-200 hover:bg-blue-50'}`}
                ...
                {quizAnswers[quizStep] !== null && (
                  <div className={`mt-4 font-semibold ${quizAnswers[quizStep] === questions[quizStep].correctAnswer ? 'text-green-700' : 'text-red-700'}`}>
                    {quizAnswers[quizStep] === questions[quizStep].correctAnswer ? 'Correct! 🎉' : 'Incorrect.'}
                    {questions[quizStep].explanation && (
                      <div className="text-sm mt-1 text-gray-600">{questions[quizStep].explanation}</div>
                    )}
                  </div>
                )}
              ...
              <div className="text-center">
                ...
                {badgeEarned ? (
                  <div className="mb-2 text-green-700 font-semibold flex flex-col items-center">
                    <span className="text-4xl mb-1">🏅</span>
                    Congratulations! You earned a badge for scoring {quizScore} out of {questions.length}!
                  </div>
                ) : (
                  <div className="mb-2 text-yellow-700 font-semibold">Score at least 45/50 to earn a badge.</div>
                )}
                <div className="mt-4">
