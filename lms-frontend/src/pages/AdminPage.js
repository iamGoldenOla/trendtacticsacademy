    ...
    return (<div className="container mx-auto py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded shadow p-6">
          ...
          {showUserModal && (<div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
              <div className="bg-white p-8 rounded shadow-lg w-full max-w-2xl">
                <h3 className="text-xl font-bold mb-4">User Management</h3>
                {loading ? (<p>Loading users...</p>) : error ? (<p className="text-red-600">{error}</p>) : (<div className="overflow-y-auto max-h-80">
        ...
        <div className="bg-white rounded shadow p-6">
          ...
          <div className="overflow-y-auto max-h-60">
            <table className="min-w-full border">
              ...
              <tbody>
                {courses.map((course, idx) => (<tr key={idx}>
                    <td className="border px-2 py-1">
                      {editIdx === idx ? (<input type="text" value={editCourseName} onChange={e => setEditCourseName(e.target.value)} className="border rounded px-1"/>) : (course.name)}
                    </td>
                    <td className="border px-2 py-1">
                      {editIdx === idx ? (<input type="text" value={editCourseDesc} onChange={e => setEditCourseDesc(e.target.value)} className="border rounded px-1"/>) : (course.desc)}
                    </td>
