-- ============================================================================
-- FIX CRITICAL ORDERING V3 (Vibe Coding) - BULLETPROOF PARKING
-- ============================================================================
...
BEGIN
    ...
    -- STEP 1: PARK MODULES TO 5000+ (ensures no conflicts)
    UPDATE modules SET ordering = ordering + 5000 WHERE course_id = cid;
    
    -- STEP 2: PARK LESSONS TO 8000+ (ensures no conflicts)
    UPDATE lessons SET ordering = ordering + 8000 WHERE course_id = cid;

    -- STEP 3: FIX MODULE ORDERING (now slots 1-8 are guaranteed empty)
    ...
    -- STEP 4: FIX LESSON ORDERING (slots are now empty)
    ...
    RAISE NOTICE 'SUCCESS: Modules 1-8 and all lessons re-ordered correctly';
END $$;
