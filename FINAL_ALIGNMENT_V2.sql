-- ============================================================================
-- FINAL CURRICULUM ALIGNMENT V2: Conflict-Free & Content-Rich
-- ============================================================================
-- FIX 1: Resolves "unique constraint" error by temporarily resetting ordering.
-- FIX 2: Deletes duplicates by selecting only the BEST single copy of each lesson.
-- FIX 3: INJECTS ALL MISSING DATA (Quizzes, Cases, Playground, Videos).
-- ============================================================================
...
BEGIN
    ...
    -- 2. RESET ORDERING (CRITICAL FIX)
    -- ========================================
    -- Move ALL lessons to a temporary high number to prevent collision errors like (1 already exists).
    UPDATE lessons SET ordering = ordering + 1000 WHERE course_id = cid;


    -- 3. ALIGN LESSONS (Select Single Best Copy & Move to Position)
    -- ========================================
    -- We select ONE ID for each title (the most recently created one) into the correct slot.
    ...
    -- ========================================
    -- Any lesson that still has ordering > 999 was NOT selected above. It is a duplicate or unwanted.
    DELETE FROM lessons WHERE course_id = cid AND ordering > 999;
    
    -- Cleanup bad modules (like "Facebook Ads")
    DELETE FROM modules WHERE course_id = cid AND id NOT IN (m1_id, m2_id, m3_id, m4_id, m5_id, m6_id);


    -- 5. MASTER CONTENT INJECTION (The "AI Trigger")
    -- ========================================
    -- Ensures EVERY lesson has the 5 Key Elements.
...
SELECT m.title as Module, l.title as Lesson, l.ordering, 
       ...
       CASE WHEN l.quizzes IS NOT NULL THEN '✅ Quiz' ELSE '❌ NO QUIZ' END as Quiz
FROM lessons l
