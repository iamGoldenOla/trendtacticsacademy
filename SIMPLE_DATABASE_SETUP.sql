...
-- This script uses basic SQL commands without complex DO blocks

-- 1. Create the student_course_access table (if it doesn't exist)
...
-- 2. Create indexes for performance (if they don't exist)
...
-- 4. Create Row Level Security policies (these will replace if exist)
...
-- 5. Create the function to get student courses (will replace if exists)
...
-- 6. Create trigger function to update timestamps (will replace if exists)
...
-- 7. Create trigger to automatically update updated_at (will replace if exists)
CREATE OR REPLACE TRIGGER update_student_course_access_updated_at 
    BEFORE UPDATE ON student_course_access 
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

-- 8. Grant permissions (these are safe to run multiple times)
...
-- 10. Show the policies that were created
SELECT 
    policyname,
    permissive,
    roles,
...
-- 11. Confirm the function exists
SELECT 
